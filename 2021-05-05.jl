...
vectorfield(f, 
			...
			norm=normalize,     # Use norm=normalize to make all arrows the same length
...
let
	plt = vectorfield(pol, params=1.0, norm=identity, scale=0.15 )
	plot!(plt, xlims=(0,1.0), ylims=(0,1.0))
	plot!(plt, t -> 1 - t, xlims=(0,1.5), ylims=(0,1.5), lc=:purple, lw=2.0, label="nullcline")
	hline!(plt, [0], lc=:red, lw=2.0, label="nullcline")
	vline!(plt, [0], lc=:blue, lw=2.0, label="nullcline")
end
...
let
	...
	plot!(plt2, t -> 1 - t, label=false, lc=:purple)
end
